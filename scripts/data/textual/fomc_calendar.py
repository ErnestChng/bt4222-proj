...
def retrieve_fomc_calendar() -> pd.DataFrame:
    ...
    #### Retrieve FOMC meeting dates from 2015 to 2020 ####
    ...
    for content in contents:
        year = content.find('h4').get_text()[:4]
        months = content.find_all('div', {"class": "fomc-meeting__month"})
        dates = content.find_all('div', {"class": "fomc-meeting__date"})
        print(f'YEAR: {year} - {len(dates)} meetings found.')
        ...
        for (month, date) in zip(months, dates):
            ...
            month_name = month.get_text().strip()
            ...
            meeting_date_str = f'{year}-{month_name}-{date_text}'
    ...
    for year in range(START_YEAR, 2015):
        ...
        for panel_heading in panel_headings:
            ...
            meeting_date_str = f'{date_text_ext[4]}-{date_text_ext[0]}-{date_text_ext[2]}'
    ...
    df = pd.DataFrame(all_data).sort_values(by=['date'])
